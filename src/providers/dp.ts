...
export class DnspodProvider extends HmacProviderBase {
  ...
  async createTxtRecord(ctx: DnsProviderContext, record: TxtRecordInput): Promise<void> {
    const { domain, subDomain } = splitDomain(record.fulldomain)
    ctx.logger.debug(`dp: creating TXT ${subDomain}.${domain} = ${record.txtvalue}`)
  ...
  async deleteTxtRecord(ctx: DnsProviderContext, record: TxtRecordInput): Promise<void> {
    const { domain, subDomain } = splitDomain(record.fulldomain)
    ctx.logger.debug(`dp: deleting TXT ${subDomain}.${domain}`)
    ...
    if (!match) {
      ctx.logger.debug(`dp: TXT record not found, nothing to delete`)
    ...
    try {
      ...
      ctx.logger.debug(`dp: delete failed, treating as success`)
    }
  }
}
