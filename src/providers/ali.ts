...
export class AliyunProvider extends HmacProviderBase {
  ...
  async createTxtRecord(ctx: DnsProviderContext, record: TxtRecordInput): Promise<void> {
    ...
    ctx.logger.debug(`ali: creating TXT ${rr}.${domainName} = ${record.txtvalue}`)
  ...
  async deleteTxtRecord(ctx: DnsProviderContext, record: TxtRecordInput): Promise<void> {
    const { domainName, rr } = splitAliDomain(record.fulldomain)
    ctx.logger.debug(`ali: deleting TXT ${rr}.${domainName}`)
    ...
    if (!match) {
      ctx.logger.debug(`ali: TXT record not found, nothing to delete`)
      return
    }
    ...
    try {
      ...
      ctx.logger.debug(`ali: delete failed, treating as success`)
    }
  }
}
