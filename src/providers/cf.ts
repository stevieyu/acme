...
export class CloudflareProvider extends HttpProviderBase {
  ...
  async createTxtRecord(ctx: DnsProviderContext, record: TxtRecordInput): Promise<void> {
    ...
    ctx.logger.debug(`cf: zone ${zoneId} for ${record.fulldomain}`)

    // Check if record already exists
    const existing = await this.findExistingRecord(zoneId, record.fulldomain, record.txtvalue)
    if (existing) {
      ctx.logger.debug(`cf: TXT record already exists`)
  ...
  async deleteTxtRecord(ctx: DnsProviderContext, record: TxtRecordInput): Promise<void> {
    ...
    if (!recordId) {
      ctx.logger.debug(`cf: TXT record not found, nothing to delete`)
    ...
    try {
      ...
      ctx.logger.debug(`cf: delete failed, treating as success`)
    }
  }
