...
import { _signWithJwk, _signWithKid } from '../../src/crypto/jws.ts'
import { _durl_replace_base64 } from '../../src/crypto/digest.ts'

describe('JWS', () => {
  it('signs with JWK thumbprint header (EC P-256)', async () => {
    const kp = await _createkey('ec-256')
    const jws = await _signWithJwk(
    ...
    const header = JSON.parse(new TextDecoder().decode(_durl_replace_base64(jws.protected)))
